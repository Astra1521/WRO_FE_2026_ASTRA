...
import numpy as np
import time
import config # Ensure this matches your import structure
...
def initialize():
    """Initializes the Picamera2 with high-performance racing settings."""
    global picam2
    try:
        picam2 = Picamera2()
        cam_config = picam2.create_preview_configuration(
            ...
            controls={
                "ExposureTime": 8000, # Locked exposure prevents color shifting
                "FrameRate": 56,      # High framerate for smooth control loops
            }
        )
...
def capture_frame():
    """Captures and returns a single frame from the camera's fast lores stream."""
    if picam2:
...
def find_objects_in_rois(frame, detection_jobs):
    ...
    This is your advanced multi-region vision architecture.
    """
...
def find_biggest_block(frame):
    """
    Legacy support function for block dodging.
    ...
    contours_red, _ = cv2.findContours(red_mask_cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours_green, _ = cv2.findContours(green_mask_cleaned, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contours, color in [(contours_red, "red"), (contours_green, "green")]:
        for cnt in contours:
            ...
            valid_blocks.append({
                "contour": cnt, "color": color, "area": area, "centroid": (center_x, center_y),
            })
    ...
    cv2.putText(overlay_frame, largest_block["color"].upper(), (x, y - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, box_color, 2)
...
if __name__ == "__main__":
    print("--- Starting Advanced Camera Vision Toolkit ---")
    if not initialize():
        ...
        try:
            print("Displaying camera feed. Move mouse over the frame to see pixel RGB and HSV values.")
            ...
            while True:
                ...
                # We use find_biggest_block here so the test tool perfectly matches your old debugger
                block_data, overlay, hsv_frame = find_biggest_block(full_frame)

                # --- Mouse Hover Debugger Implementation ---
                if (
                    ...
                    # Print values directly onto the video feed
