...
def initialize():
    ...
    global servo_pwm
    try:
        # Force GPIO 12 into PWM mode for the Raspberry Pi 5
        subprocess.run(["pinctrl", "set", "12", "a0"], check=True)
        
        # rpi_hardware_pwm uses the channel number (Channel 0 = GPIO 12, Channel 1 = GPIO 13)
...
def angle_to_duty(absolute_angle: float):
    """
    Helper function from your test code. 
    Converts a 0-180 degree angle into the correct PWM duty cycle.
    """
    # Safety clamp absolute angle between 0 and 180 for the math formula
    ...
    # Convert microseconds to duty cycle % (20,000us = 20ms period for 50Hz)
...
def set_angle(input_angle: float):
    ...
    # 1. Software Clamp: Limit FSM input to -45 / +45 degrees
    clamped_input = max(-45.0, min(45.0, input_angle))

    # 2. Convert to Absolute Hardware Angle (Center is 95)
    # If input is 0 -> 95. If input is -45 -> 50. If input is +45 -> 140.
    absolute_angle = config.SERVO_CENTER + clamped_input

    # 3. Hardware Clamp: Ensure we never exceed physical servo limits
...
def set_angle_unlimited(input_angle: float):
    ...
    # Skip the -45/+45 clamp and directly calculate absolute angle
    absolute_angle = config.SERVO_CENTER + input_angle

    # Still strictly enforce physical hardware safety limits (50 to 140)
    absolute_angle = max(config.SERVO_MIN, min(config.SERVO_MAX, absolute_angle))
